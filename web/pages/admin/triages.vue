<template>
  <div>
    <div class="flex items-center gap-3 mb-4 flex-wrap">
      ...
      <span class="text-xs text-stone-400" v-if="data">{{ data.pagination.total }} triages</span>
    </div>
    ...
    <div v-if="data?.stats" class="flex items-center gap-4 mb-3 px-3 py-2 bg-white border border-stone-200 rounded-lg text-xs">
      ...
      <div v-if="data.stats.avgCostCents" class="text-stone-400">
        avg <span class="font-mono">${{ (data.stats.avgCostCents / 100).toFixed(2) }}</span>/triage
      </div>
    ...
    <div v-if="data" class="bg-white border border-stone-200 rounded-lg overflow-hidden">
      <table class="w-full text-xs">
        ...
        <tbody>
          <tr v-for="t in data.triages" :key="t.id"
            :class="t.status === 'failed' ? 'border-b border-red-100 bg-red-50/30 hover:bg-red-50/60' : 'border-b border-stone-50 hover:bg-stone-50/50'">
            <td class="py-2 px-3">
              <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium"
                :class="{
                  'bg-emerald-50 text-emerald-700': t.status === 'complete',
                  'bg-yellow-50 text-yellow-700': t.status === 'processing',
                  'bg-red-50 text-red-700 cursor-pointer': t.status === 'failed',
                }"
                @click="t.status === 'failed' && toggleExpand(t.id)">
                <span class="w-1 h-1 rounded-full"
                  :class="{ 'bg-emerald-500': t.status === 'complete', 'bg-yellow-500': t.status === 'processing', 'bg-red-500': t.status === 'failed' }"></span>
                {{ t.status }}
                <svg v-if="t.status === 'failed' && t.failReason" class="w-2.5 h-2.5 transition-transform" :class="expandedRows.has(t.id) && 'rotate-180'" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"/>
                </svg>
              </span>
              <div v-if="t.status === 'failed' && expandedRows.has(t.id) && t.failReason"
                class="mt-1 p-1.5 rounded bg-red-50 text-[10px] text-red-600 font-mono break-all max-w-xs max-h-32 overflow-auto whitespace-pre-wrap">{{ t.failReason }}</div>
            </td>
            <td class="py-2 px-3">
              <a v-if="t.status === 'complete'" :href="`/triage/${t.slug}`" target="_blank"
                class="font-mono text-stone-600 hover:text-stone-900 hover:underline">{{ t.slug }}</a>
              <span v-else class="font-mono text-stone-400">{{ t.slug }}</span>
            </td>
            <td class="py-2 px-3 text-stone-600 max-w-[200px] truncate">{{ t.filename || '-' }}</td>
            <td class="py-2 px-3 text-right font-mono text-stone-900">${{ (t.costCents / 100).toFixed(2) }}</td>
            <td class="py-2 px-3 text-right font-mono text-stone-400">{{ t.durationMs ? formatDuration(t.durationMs) : '-' }}</td>
            <td class="py-2 px-3 text-stone-400 font-mono whitespace-nowrap">{{ formatTime(t.createdAt) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
async function fetchData() {
  try {
    ...
    data.value = await $fetch('/api/admin/triages', { query })
  } catch {
    error.value = true
  }
}
